...
df = pd.read_csv('/Users/Shatalov/Downloads/Lab2_6-main/fancy.csv', sep=';')
print (df.values[0, 0])


df = pd.read_table('wine_Austral.dat', sep='\t')
print (df.values[:, 6])
series = Series(df.values[:, 6])
...
def plot_initial():  # Initial time sequence
    plt.plot(initial_time, df.values[:, 6])
    plt.xlabel("Month")
    plt.ylabel("Sales")
...
x, y = np.reshape(initial_time, (-1, 1)), df.values[:, 6]
regression.fit(x, y)

result = regression.predict(np.reshape(initial_time, (-1, 1)))

plt.scatter(initial_time, df.values[:, 6].flatten(),  color='black')
...
in_time = parser.parse(str(df.values[0, 9]))
print("result\n==============\n")
